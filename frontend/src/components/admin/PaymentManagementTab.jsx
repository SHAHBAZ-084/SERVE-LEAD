...
export const getFeeApprovalBadge = (m) => {
  ...
  if (m.feeStatus === "waived") return { label: "Free", cls: "bg-purple-100 text-purple-700 border-purple-200" };
  if (m.feeStatus === "not_requested") return { label: "No Fee Yet", cls: "bg-amber-100 text-amber-700 border-amber-200" };
  return null;
};
...
export const canWaiveExecutive = (app) =>
  !app.feeWaived && app.status === "pending";
...
const PaymentManagementTab = ({ pendingMembers, fetchPendingMembers, auth, notify, api, Spinner, inputCls }) => {
  ...
  const handleRequestFee = async (memberId) => {
    setProcessing(true);
    try {
      const r = await api.post(`fees/request/${memberId}`, {
        amount: Number(membershipFee) || 0,
